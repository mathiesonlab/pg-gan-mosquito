...
import global_vars
import param_set
import real_data_random_updated
import simulation
...
def process_gt_dist(gt_matrix, dist_vec, region_len=False, real=False,
    ...
    if mid >= half_S:
        # don't do major/minor if unphased
        if phased:
            region[:,:,0] = major_minor(gt_matrix[mid-half_S:mid+
                other_half_S,:].transpose(), neg1)
        else:
            region[:,:,0] = gt_matrix[mid-half_S:mid+other_half_S,:].transpose()
    ...
    # allow for unphased data
    if not phased:
        region = collapse_phase(region)

    return region # n X SNPs X 2

def collapse_phase(region):
    """Turn msprime simulations into unphased data"""
    n, S, _ = region.shape
    assert n % 2 == 0

    # set up new unphased region
    unphased = np.zeros((n//2, S, 2))

    for i in range(0,n//2):
        # take two individuals from the first channel, second channel is the
        # same for all individuals
        snps = np.sum(region[2*i:2*i+2,:,0], axis=0)
        distances = region[2*i,:,1]
        unphased[i] = np.vstack((snps, distances)).transpose()
...
def major_minor(matrix, neg1):
    ...
    # kind of a hack: make triallelic sites biallelic
    # (okay since we collapse phased after this)
    matrix[matrix > 1] = 1 # we filter in the VCF too
...
def parse_args(in_file_data = None, param_values = None):
    ...
    parser.add_option('-u', action="store_true", dest="unphased",
        help='unphased')
...
def parse_sample_sizes(n_string, phased=True):
    """ n_string is e.g. "12,6,8" """
    raw = [int(n) for n in n_string.split(",")]

    if phased:
        return raw

    # if data is unphased, make sure we have an even number of haps, then divide
    for n in raw:
        assert n % 2 == 0
    return [n//2 for n in raw]

def process_opts(opts, summary_stats = False):

    phased = not opts.unphased
    sample_sizes = parse_sample_sizes(opts.sample_sizes, phased=phased)
    ...
    if opts.data_h5 is not None: # h5 is None option at end of func
        ...
        iterator = real_data_random_updated.RealDataRandomIterator(opts.data_h5,
            opts.seed, opts.bed, phased=phased)
        assert iterator.num_samples == sum(sample_sizes)
    ...
    gen = generator.Generator(simulator, param_names, sample_sizes,
        opts.seed, mirror_real=real, reco_folder=opts.reco_folder,phased=phased)

    if opts.data_h5 is None:
        # "real data" is simulated with fixed params
        iterator = generator.Generator(simulator, param_names, sample_sizes,
            opts.seed, phased=phased) # don't need reco_folder
