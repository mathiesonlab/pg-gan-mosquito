...
def process_gt_dist(gt_matrix, dist_vec, region_len=False, real=False,
    ...
    print(np.unique(gt_matrix, return_counts=True))
...
def tri_allelic(matrix):
    """
    Filter tetra-allelic sites and convert tri-allelic to -1,0,1 using:
       -1: majority allele
        0: second most common allele
        1: minority allele
    Note: matrix should be num_snps x num_haps (pre-transpose)
    """
    map_to = [-1, 0, 1]
    allele_map = {} # map from 0,2,1 (for example) to -1,0,1
    snp_lst = [] # not sure how many we will have
    num_snps, num_haps = matrix.shape

    # loop through SNPs
    for j in range(num_snps):
        alleles, counts = np.unique(matrix[j], return_counts=True)
        if len(alleles) <= 3: # bi- or tri-allelic only
            # sort by frequency and replace with dictionary
            order = np.argsort(counts)[::-1] # sort high to low freq
            for i in range(len(map_to)):
                allele_map[order[i]] = map_to[i]
            idx = np.nonzero(allele_map.keys() == matrix[j][:,None])[1]
            encoded = np.asarray(allele_map.values())[idx]
            snp_lst.append(encoded)
            
    return np.array(snp_lst)
